...
Udacity CarND-Behaviorial-Cloning-Project
==========================================



This repository contains the completed CarND-Behavioral-Cloning Project, which demonstrates
the use of a deep neural network to clone driving behavior. A car is driven around a track in a simulator 
program and video from a simulated front mounted camera is saved as a series of images along with 
the steering angle of the car. 
The images provide the input training data for a neural network whose target is the steering angle. 

The simulator is then run in autonomous mode, using the trained network to predict the steering angle 
based on the camera image at each time step, and thus drive the car around the track. 
The speed/throttle is user controlled in driving mode (where images/steering angles are recorded), but 
is automatically controlled in autonomous mode to a constant speed set in drive.py.    



## List of Files
- README.md          -- this file.
- BHC_WriteUp.md     -- Description of techniques and code used for this project and how and 
			what training data was generated and used.
- final_run.mp4      -- Video (camera view) of autonomous driving of the car around the track, about two laps.
  		     	Video is 48 frames per second.
- ./final_run.tar.gz -- Tarball of images used to generate final_run.mp4 video.
- model.py           -- Code that ingests a series of training camera images and trains a neural network 
  		     	to predict the steering angle of the car. Outputs model.h5.
- model.h5           -- Neural network model (HDF5 format) generated by model.py, used by drive.py
  		        in autonomous driving mode.
- drive.py           -- Python code that uses model.h5 to predict the steering angle in autonomous mode.
  		        This version is unaltered from that downloaded from the original project repository.
			Speed is controlled to a constant 9 mph.
- examples/          -- Directory of images referred to in BHC_Writeup.md.
